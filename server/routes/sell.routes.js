            ...
            connection.beginTransaction(function (error) {
                ...
                connection.query(orderQuery, invoice, function (error, result) {
                    ...
                    let invoice_map = Array.from(items).map(function (item) {
                        return [invoice_ID, item.item_ID, invoice.invoice_type, item.qty, item.currency, item.exchange_rate, item.unit_cost, item.unit_price]
                    });
                    let mapQuery = `INSERT INTO invoice_map (invoice_ID_FK, item_ID_FK, record_type, qty, currency, exchange_rate, unit_cost, unit_price) VALUES ?`;
